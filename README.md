...
| `/api/players/statistics` | GET | Request param: sortingType optional, enum (points, wins,loses, draws, gamesPlayed, winsVsAi).<br>Request param: sortingOrder optional, enum (asc, desc) |  Returns sorted statistics for all the players | List of PlayerDTO | 200(ΟΚ) |
...
| `/api/games` | GET |  | Rerurns all available games to join | List of GameResponseDTO | 200(OK) |
